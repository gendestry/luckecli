...
#include "Client.h"
#include "Display/Components/InfoPanel/ApplyRevertBar.h"
...
namespace Test
{
    ...
    public:
        ...
        void sync(const Client &client, const std::string &ip, int fixtureId);

        // Render the form for `client.fixtures[fixtureId]`.
        ftxui::Element render(const Client &client, const std::string &ip, int fixtureId) const;
