...
#include "core/commands/legacy/v96.h"
#include "core/commands/legacy/v98.h"
...
namespace core::legacy
{
    ...
    public:
        ...
        static bool describe(const std::optional<std::string> &resp, core::Client &client)
        {
            if (!resp || resp->empty())
                return false;
        ...
        static void fetchPresets(core::Client &client, ESPClient &conn)
        {
            for (auto &fix : client.fixtures)
            {
                auto pr = conn.sendRequestOpt(
                    support::JSONtemp::stringify("getfixture", "id", fix.id, "value", "presets"), 4000ms);
                if (pr)
        ...
        static bool presets(const std::optional<std::string> &resp, const core::Client &client,
