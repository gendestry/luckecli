...
void CommandExecutor::bindCommands() {
    ...
    m_registry.add("wirelessprint", "Get/set the wireless report task",
                   ...
                   [this](const Args &a) {
                       return taskConfig(a, "wirelessprint",
                                         "wireless_report_task",
...
bool CommandExecutor::help(const Args &args) {
    if (args.has(1)) {
        ...
        log.println("  {}usage:{} {}{}{}", Theme::lbl(), Theme::r(),
                    Theme::val(), cmd->usage, Theme::r());
        return true;
    }
    ...
    for (const auto &group : m_registry.getGroups()) {
        ...
        for (const auto &cmd : group.commands) {
            if (!cmd->isUsable(mode()) &&
                !cmd->isUsable(Command::Usable::ANYTIME))
                continue;
            if (!printedHeader) {
                log.println("{}{}{}", Theme::heading(), group.name, Theme::r());
                printedHeader = true;
            }
            log.println("  {}{}{}  {}{}{}", Theme::name(), cmd->name(),
                        Theme::r(), Theme::dim(), cmd->desc, Theme::r());
        }
    }
    return true;
}
    ...
    for (const auto &[ip, client] : devices) {
        ...
        for (const auto &fx : client.fixtures) {
            log.println("    {}[{}]{} {}{}{}  {}{}{}", Theme::val(), index++,
                        Theme::r(), Theme::name(), fx.name, Theme::r(),
...
bool CommandExecutor::select(const Args &args) {
    ...
    m_selection.set(std::move(picked));
...
void CommandExecutor::selectAll() {
    m_selection.set(flatten(m_sharedState));
...
bool CommandExecutor::setpreset(const Args &args) {
    // Firmware field is "presetIndex"; the command is the friendlier
    // "setpreset".
...
bool CommandExecutor::setname(const Args &args) {
    ...
    // Success: patch the cached fixture so a later `describe`/`list` reflects
    // it.
    m_sharedState.withClient(sel.ip, [&](Client &c) {
        if (sel.fixtureId >= 0 &&
            sel.fixtureId < static_cast<int>(c.fixtures.size()))
...
bool CommandExecutor::blackout(const Args &) {
    ...
    const Utils::Network::IP localIp = Utils::Network::Interfaces::primaryIP();
    const std::array<uint8_t, 512>
        zeros{}; // value-initialised → all channels 0
    ...
    if (cleared == 0)
        return false;
    log.println("{}✓{} blackout — cleared {}{}{} universe(s) via {}{}{}",
                Theme::ok(), Theme::r(), Theme::val(), cleared, Theme::r(),
                Theme::ip(), localIp.str(), Theme::r());
    return true;
}

// setcolor <r> <g> <b>: send an RGB color over sACN to the selected fixtures
// (or all of them if nothing is selected). Each fixture's footprint is filled
...
bool CommandExecutor::sendBuffers(
    ...
    // One sACN sender per universe: each targets its own multicast group and
    // owns its own sequence counter.
    const Utils::Network::IP localIp = Utils::Network::Interfaces::primaryIP();
    std::size_t sent = 0;
...
bool CommandExecutor::setwifi(const Args &args) {
    ...
    // Device-level: a single target only (a multi-device selection is
    // ambiguous).
    auto ips = selectedIps();
    if (ips.size() != 1) {
    ...
    log.println("{}Sending wifi config to {}{}{}...", Theme::dim(), Theme::ip(),
                ip, Theme::r());
    auto resp =
        m_sharedState.getESPClient(ip)->sendRequestOpt(r.dump(), 4000ms);
    if (!resp) {
...
bool CommandExecutor::refreshDevice(const std::string &ip) {
    ...
    for (int attempt = 0; attempt < 3 && !resp; ++attempt) {
        ...
        resp = client->sendRequestOpt(support::JSONtemp::stringify("describe"),
                                      4000ms);
    }
    if (!resp) {
...
bool CommandExecutor::presets(const Args &) {
    ...
    for (const auto &sel : m_selection) {
        auto client = m_sharedState.getESPClient(sel.ip);
        auto resp = client->sendRequestOpt(
            support::JSONtemp::stringify("getfixture", "id", sel.fixtureId,
                                         "value", "presets"),
            4000ms);
        if (!resp) {
            log.error("No response from {} fixture {}", sel.ip, sel.fixtureId);
        ...
        log.println("{}{}{} {}{}{}", Theme::ip(), sel.ip, Theme::r(),
                    Theme::name(), fix.name, Theme::r());
...
bool CommandExecutor::exportgdtf(const Args &args) {
    ...
    log.println("{}exported{} {} {}({} channels = {} RGB cells){}", Theme::ok(),
                Theme::r(), path, Theme::dim(), fix.footprint,
                fix.footprint / 3, Theme::r());
    return true;
}
...
bool CommandExecutor::taskConfig(const Args &args, const std::string &cmdName,
    ...
    auto ips = selectedIps();
    if (ips.size() != 1) {
        log.error("{} targets a single device (selected {})", cmdName,
                  ips.size());
        return false;
    }
    const std::string &ip = ips.front();
    ...
    if (cache)
        m_sharedState.withClient(ip,
                                 [&](Client &c) { c.engine.*cache = current; });
...
void CommandExecutor::okFixture(const Selection &sel, const std::string &label,
    ...
    log.println("{}✓{} {}{}{} {}→{} {}{}{}  {}{}{} fixture {}{}{}", Theme::ok(),
                ...
                Theme::r(), Theme::val(), value, Theme::r(), Theme::ip(),
                sel.ip, Theme::r(), Theme::val(), sel.fixtureId, Theme::r());
}
